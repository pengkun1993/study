...
class ExampleController extends AdminCoreController {
	...
	protected function _search() {
		...
		/* 名称：所属栏目 字段：pid 类型：select*/
		if($post_data['s_pid']!=''){
			$map['pid']=$post_data['s_pid'];
		}
		/* 名称：图片 字段：picture 类型：pictures*/
		if($post_data['s_picture']!=''){
			$map['picture']=$post_data['s_picture'];
		}
		...
		/* 名称：显示顺序呢 字段：sort 类型：string*/
		if($post_data['s_sort']!=''){
			$map['sort']=array('like', '%'.$post_data['s_sort'].'%');
		}
		return $map;
	}
	...
	public function add(){
		if(IS_POST){
			...
			if($data){
				...
				if($result){
					action_log('Add_Example', 'Example', $result);
	...
	public function edit(){
		if(IS_POST){
			...
			if($data){
				...
				if($result){
					action_log('Edit_Example', 'Example', $post_data['id']);
					$this->success ( "操作成功！",U('index'));
				}else{
					$error = $this->Model->getError();
					$this->error($error ? $error : "操作失败！");
	...
	public function del(){
		...
		if(!$res){
			...
			action_log('Del_Example', 'Example', $id);
			$this->success('删除成功！');
		}
	}
}