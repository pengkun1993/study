...
class MenuController extends AdminCoreController {
	...
	public function index(){
		if(IS_POST){
			...
			$map = array();
        	//根据搜索条件对模型数据进行搜索
			$_list = $this->Model->where($map)->order('sort asc')->getField('id,pid,catname,sort,icon');
		...
		if(IS_POST){
			...
			if($data){
				...
				if($result){
					...
					action_log('Add_Menu', 'Menu', $result);
                    //返回成功信息
		...
		if(IS_POST){
			...
			if($data){
				...
				if($result){
                	//修改成功
                    //通过行为执行操作，（如果数据库没有定义此行为不进行任何操作）
					action_log('Edit_Menu', 'Menu', $post_data['id']);
                    //返回成功信息
					$this->success ( "操作成功！",U('index'));
				}else{
                	//修改失败
