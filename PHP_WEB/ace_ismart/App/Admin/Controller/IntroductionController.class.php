...
class IntroductionController extends AdminCoreController {
	...
	public function add(){
		if(IS_POST){
			...
			if($data){
				...
				if($result){
					action_log('Add_Introduction', 'Introduction', $result);
	...
	public function edit(){
		if(IS_POST){
			...
			if($data){
				...
				if($result){
					action_log('Edit_Introduction', 'Introduction', $post_data['id']);
					$this->success ( "操作成功！",U('index'));
				}else{
					$error = $this->Model->getError();
					$this->error($error ? $error : "操作失败！");
	...
	public function del(){
		...
		if(!$res){
			...
			action_log('Del_Introduction', 'Introduction', $id);
			$this->success('删除成功！');
		}
	}
}