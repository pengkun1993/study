...
class <?php echo ($ModelInfo['name']); ?>Controller extends AdminCoreController {
	...
	public function add(){
		if(IS_POST){
			$post_data=I('post.');
<?php  foreach($_Add as $field_key=>$field){ if(in_array($field['type'],array('select','checkbox'))){ $extra=unserialize($field['extra']); if($extra['multiple']==1){ echo '$post_data["'.$field["name"].'"]=I("post.'.$field["name"].'");'; echo '$post_data["'.$field["name"].'"]=implode(",",$post_data["'.$field["name"].'"]);'; } } } ?> 
	...
	public function edit(){
		if(IS_POST){
			$post_data=I('post.');
<?php  foreach($_Edit as $field_key=>$field){ if(in_array($field['type'],array('select','checkbox'))){ $extra=unserialize($field['extra']); if($extra['multiple']==1){ echo '$post_data["'.$field["name"].'"]=I("post.'.$field["name"].'");'; echo '$post_data["'.$field["name"].'"]=implode(",",$post_data["'.$field["name"].'"]);'; } } } ?> 
			$data=$this->Model->create($post_data);
			if($data){
