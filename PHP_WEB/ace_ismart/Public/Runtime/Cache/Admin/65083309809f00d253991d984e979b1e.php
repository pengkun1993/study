...
<div class="fixed-bar" id="Introduction_Bar">
	<div class="item-title">
		<h3>公司简介</h3>
		<ul class="tab-base">
			<li><a class="current" href="JavaScript:void(0);" onclick="Data_Reload('Introduction_Data_List');"><span>列表</span></a></li>
			<li><a href="JavaScript:void(0);" onclick="Data_Search('Introduction_Search_From','Introduction_Data_List');"><span>搜索</span></a></li>
			<?php if(Is_Auth('Admin/Introduction/add')): ?><li><a href="<?php echo U('Admin/Introduction/add');?>"><span>新增</span></a></li><?php endif; ?>
		</ul>
	</div>
</div>
...
$(function() {
	$("#Introduction_Data_List").datagrid({
		url : "<?php echo U('Introduction/index');?>",
		fit : true,
		striped : true,
		border : false,
		pagination : true,
...
{field : "content",title : "公司简介",width :100,sortable:true},{field : "state",title : "状态",width :100,sortable:true,formatter: function (value, row, index) {
			...
			return op_state[value];
				...
				<?php if(Is_Auth('Admin/Introduction/edit')): ?>operate_menu = operate_menu+"<a href='<?php echo U('edit'); ?>&id="+row.id+"' >编辑</a>";<?php endif; ?>

				<?php if(Is_Auth('Admin/Introduction/del')): ?>operate_menu = operate_menu+" | <a href='#' onclick=\"Data_Remove('<?php echo U('del'); ?>&id="+row.id+"','Introduction_Data_List');\">删除</a>";<?php endif; ?>
