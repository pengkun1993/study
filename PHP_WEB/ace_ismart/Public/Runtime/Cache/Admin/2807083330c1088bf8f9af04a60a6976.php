...
{field : "action_id",title : "行为id",width :100,sortable:true},{field : "user_id",title : "执行用户id",width :100,sortable:true},{field : "create_time",title : "执行行为的时间",width :150,sortable:true,formatter: function (value, row, index) {
			return u_to_ymdhis(value)
				...
				<?php if(Is_Auth('Admin/ActionLog/edit')): ?>operate_menu = operate_menu+"<a href='<?php echo U('edit'); ?>&id="+row.id+"' >查看详情</a>";<?php endif; ?>

				<?php if(Is_Auth('Admin/ActionLog/del')): ?>operate_menu = operate_menu+" | <a href='#' onclick=\"Data_Remove('<?php echo U('del'); ?>&id="+row.id+"','ActionLog_Data_List');\">删除</a>";<?php endif; ?>
