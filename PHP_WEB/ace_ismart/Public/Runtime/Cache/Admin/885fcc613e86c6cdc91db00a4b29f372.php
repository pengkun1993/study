	...
	<div class="item-title">
		<div class="page-header">
			<h1>行为				<small>
					...
					<?php if(Is_Auth('Admin/Action/add')): ?><a class="top_a" href="<?php echo U('Admin/Action/add');?>"><span>新增</span></a><?php endif; ?>
				</small>
			</h1>
		</div>
	</div>
...
{field : "name",title : "行为唯一标识",width :100,sortable:true},{field : "title",title : "行为说明",width :100,sortable:true},{field : "remark",title : "行为描述",width :100,sortable:true},{field : "rule",title : "行为规则",width :100,sortable:true},{field : "log",title : "日志规则",width :100,sortable:true},{field : "status",title : "状态",width :100,sortable:true,formatter: function (value, row, index) {
			...
			return u_to_ymdhis(value)
				...
				<?php if(Is_Auth('Admin/Action/edit')): ?>operate_menu = operate_menu+"<a href='<?php echo U('edit'); ?>&id="+row.id+"' >编辑</a>";<?php endif; ?>

				<?php if(Is_Auth('Admin/Action/del')): ?>operate_menu = operate_menu+" | <a href='#' onclick=\"Data_Remove('<?php echo U('del'); ?>&id="+row.id+"','Action_Data_List');\">删除</a>";<?php endif; ?>
